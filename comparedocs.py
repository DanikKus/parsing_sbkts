...
def parse_fields(text):
    ...
    while i < total_lines:
        ...
        # Дата оформления
        elif "Дата оформления" in line:
            m = re.search(r"Дата оформления[:\s]+([0-9]{2}\.[0-9]{2}\.[0-9]{4})", line)
            if m:
                result["Дата оформления"] = m.group(1)
            elif i + 1 < total_lines:
                # Возможно, дата на следующей строке!
                next_line = lines[i + 1].strip()
                m = re.search(r"([0-9]{2}\.[0-9]{2}\.[0-9]{4})", next_line)
                if m:
                    result["Дата оформления"] = m.group(1)
                    i += 1
    ...
    if not result["Дата оформления"]:
        result["Дата оформления"] = extract_date_of_issue(text)
...
def split_list_from_string(val):
    if not val:
        return []
    if isinstance(val, list):  # если уже список
        return val
    # Разделяем по запятой ИЛИ точке с запятой (без пробелов)
    return [x.strip() for x in re.split(r"[,;]", val) if x.strip()]

def is_hybrid_motor_case(parsed):
    # Определяем: если есть два значения в каждом поле
...
def postprocess_ev_fields(parsed, text):
    # --- Старая логика, если надо оставить ---
    m_block = re.search(
    ...
    m1 = re.search(r"Рабочее напряжение, В\s*([\d\., ]+)", text)
    if m1:
        parsed["Рабочее напряжение, В"] = m1.group(1)

    m2 = re.search(r"Максимальная 30-минутная мощность, кВт\s*([\d\., ]+)", text)
    if m2:
        parsed["Максимальная 30-минутная мощность, кВт"] = m2.group(1)

    # --- ГИБРИД: разнести по полям! ---
    if is_hybrid_motor_case(parsed):
        ...
        # Основной двигатель (электродвигатель электромобиля) пустой (это гибрид!)
        parsed["Электродвигатель электромобиля"] = ""
...
def extract_lab_line(text):
    lines = text.splitlines()
    for i, line in enumerate(lines):
        if "ИСПЫТАТЕЛЬНАЯ ЛАБОРАТОРИЯ" in line.upper():
            # Ищем подчеркнутую строку на 1–3 строках ниже
            for j in range(i + 1, min(i + 4, len(lines))):
                if re.search(r"_{3,}", lines[j]) or lines[j].count("_") >= 3:
                    return lines[j].replace("_", "").strip()
    return None

def postprocess_ev_fields(data, text):
    def extract_lab_line(text):
        lines = text.splitlines()
        for i, line in enumerate(lines):
            if "ИСПЫТАТЕЛЬНАЯ ЛАБОРАТОРИЯ" in line.upper():
                # Ищем подчеркнутую строку в ближайших 1-3 строках
                for j in range(i + 1, min(i + 4, len(lines))):
                    next_line = lines[j].strip()
                    if re.search(r"_+", next_line) or next_line.count("_") >= 3:
                        return next_line.replace("_", "").strip()
        return None

    lab_name = extract_lab_line(text)
    if lab_name:
        data["ИСПЫТАТЕЛЬНАЯ ЛАБОРАТОРИЯ"] = lab_name

    # Удаляем ненужные поля
    data.pop("Номер аттестата", None)
    data.pop("Срок действия аттестата", None)
...
def extract_date_of_issue(text):
    # Любой вариант "Дата оформления" + дата, даже если на новой строке
    m = re.search(r"Дата оформления[:\s]*([0-9]{2}\.[0-9]{2}\.[0-9]{4})", text)
    if m:
        return m.group(1)
    # Второй вариант: "Дата оформления" на одной строке, дата на следующей
    m2 = re.search(r"Дата оформления[^\d]{0,10}\n\s*([0-9]{2}\.[0-9]{2}\.[0-9]{4})", text)
    if m2:
        return m2.group(1)
    return ""
...
if __name__ == "__main__":
    ...
    # ОСНОВНОЙ ПАРСИНГ + ОБРАБОТКА
    parsed = postprocess_ev_fields(parse_fields(txt), txt)
